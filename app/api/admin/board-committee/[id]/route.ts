...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.boardCommittee.update({ where: { id }, data });
    return NextResponse.json(updated);
  } catch (error) {
    console.error('ADMIN board-committee update error', error);
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const id = Number(params.id);
    if (!id) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
    await prisma.boardCommittee.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error('ADMIN board-committee delete error', error);
    return NextResponse.json({ error: 'Server error' }, { status: 500 }); }
}
