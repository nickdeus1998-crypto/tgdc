        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            {['all', 'operational', 'construction', 'development', 'exploration'].map((category) => (
              <button
                key={category}
                className={`filter-btn px-6 py-3 rounded-full border-2 border-primary-green text-primary-green font-semibold transition-all duration-300 ${filter === category ? 'active' : ''
                  }`}
                  ...
                  <div className="space-y-4">
                    {modalProject.timeline.map((phase) => (
                      <div key={phase.phase} className="flex items-center">
                        <div
                          className={`w-4 h-4 rounded-full mr-4 ${phase.status === 'Completed'
                              ? 'bg-green-500'
